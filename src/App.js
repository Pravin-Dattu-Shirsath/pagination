...
const onpagination=(start,end)=>{
  ...
  return (
    <div className={`App ${mode===true?"bg-dark":null}`}>
      <button className={`btn  btn-sm DRKButton ${mode===false?"btn-dark":"btn-light"}`} onClick={darkmodebtn}>{mode===false?"Dark Mode":"Ligth Mode"}</button>
       ...
       return (  <div className='  m-3' key={post.id}>
          ...
          <Pagination perPageShow={perPageShow} onpagination={onpagination} total={posts.length}/>
        </div>
